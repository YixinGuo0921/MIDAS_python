...
def find_matching_date_index(hf_data_index, lfdate):
    if lfdate in hf_data_index:
        return hf_data_index.get_loc(lfdate)
    else:
        return pd.Series(hf_data_index >= lfdate).idxmin()
...
def mix_freq(lf_data, hf_data, xlag, ylag, horizon, start_date=None, end_date=None):
    ...
    for lfdate in lf_data.loc[start_date:max_date].index:
        start_hf = find_matching_date_index(hf_data.index, lfdate)
...
def mix_freq2(lf_data, hf_data1,hf_data2, x1lag, x2lag, ylag, horizon, start_date=None, end_date=None):
    ...
    for lfdate in lf_data.loc[start_date:max_date].index:
        start_hf1 = find_matching_date_index(hf_data1.index, lfdate)
        start_hf2 = find_matching_date_index(hf_data2.index, lfdate)
